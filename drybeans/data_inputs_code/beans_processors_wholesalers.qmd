...
In this file we compile and merge data from multiple sources:

1.  Businesses that have previously sold to New York City
2.  Businesses that are located in New York from the Simply Analytics database that are processors or wholesalers/distributors of beans
...
5.  Businesses identified by the research team as key out-of-state players. 
...
We assume that any business that sells through the local supply chain (i.e., was interviewed by research team, sold to NYC previously, was part of the list of businesses that are NY Grown & Certified, or identified by research as a key out-of-state seller) should be included in the bean supply chain model. For this set of businesses, we gather missing information manually. 
...
In these data, many companies are listed multiple times if they have multiple locations. In some cases that is because the same company is performs multiple roles in the supply chain (e.g., processing and distribution). 
...
# How many duplicates have different locations in the supply chain that we care about
duplicates <- duplicates %>%
  mutate(
    supply_chain_location = case_when(
      sheet_name == "Mfg2" ~ "processor", 
      TRUE ~ "warehouse/distribution")
  ) %>%
  group_by(company_name) %>%
  summarise(
    n = n_distinct(supply_chain_location)
  ) %>% 
  filter(n>1)
...
# Summarize data for those that are duplicated and different and return first row for those that are duplicated and the same
naics_df <- naics_df %>% 
...
df <- df %>% 
  mutate(
    latitude = case_when(
      ...
      company_name=="SYSCO ALBANY" ~ 42.898884,
    ...
    longitude = case_when(
      ...
      company_name=="SYSCO ALBANY" ~ -73.768914,
...
df <- df %>% 
  select(!c(beans_processor, nyc_dashboard:last_col()))
