...
# How many duplicates have different functions on the supply chain
duplicates <- duplicates %>% 
  group_by(company_name) %>% 
  mutate(
    supply_chain_function = case_when(
      sheet_name == "Mfg2" ~ "processor", 
      TRUE ~ "wholesale/distributor")
  ) %>%
  summarise(
    n = n_distinct(supply_chain_function)
  ) %>% 
  filter(n>1)
...
# Keep only data from Wild Kale:Vermont Bean crafters (those that were interviewed)
qual_df <- qual_df %>% 
