...
# Keep leafy green operations only
df <- df %>% 
...
kable(sum_greens) %>% 
  kable_styling() %>% 
  footnote(general = "All dollar amounts are in 2022 dollars. Primary commodity: Other crops includes hay, nursery, greenhouse, christmas trees, woody crops, and other crops.")
```
...
## Fixed expenses - Leafy Green operations
...
# Create a stacked bar chart of fixed expenses as a percent of total fixed expense for conventional and organic operations, separate charts for leafy greens and cabbage operations 
...
ggplot(fig_df, aes(fill = short_variable_description, 
  ...
  theme(legend.title = element_blank(),
        axis.title.x = element_blank()) +
  scale_y_continuous(labels = percent_format(accuracy = 1)) + 
  labs(y = "Percent of total fixed expense") +
  geom_text(aes(label = label_percent(accuracy = 1)(value)), 
            position = position_stack(vjust = 0.5), 
            size = 3.5, color = "white") 
```
...
## Average profitability - Leafy Green operations
...
## Return on Assets - Leafy Green operations

```{r roa_green_boxplot, fig.align="center", fig.width=7, fig.height=6, out.width="100%"}
...
sum <- df %>% 
  ...
  mutate(
    type = case_when(
      str_detect(variable, "_conventional") ~ "Conventional", 
    str_detect(variable, "organic")  ~ "Organic"),
    variable = str_remove_all(variable, "_conventional|_organic")) %>% 
  pivot_wider(
    names_from = variable, 
    values_from = value)
...
ggplot(roa, aes(x = type, y = mean)) +
  geom_point(size = 4, shape = 23, fill = "red") + 
  ...
  labs(
    x = "", 
    y = "Rate of return on assets", 
    title = "Rate of return on assets", 
    caption = "The red diamond represents the mean and the error bars show the distribution around the mean") + 
  theme(plot.caption = element_text(hjust = 0))

```

## Net Farm Income - Leafy Green operations

```{r infi_green_boxplot, fig.align="center", fig.width=7, fig.height=6, out.width="100%"}

# Net farm income
infi <- sum %>% 
  filter(yvar=="infi")
  
ggplot(infi, aes(x = type, y = mean)) +
  geom_point(size = 4, shape = 23, fill = "red") + 
  geom_errorbar(aes(ymin = mean - se,
                    ymax = mean + se, 
                    width = 0.2)) + 
  scale_y_continuous(
    ...
    breaks = seq(-70000, 70000, by = 20000)) + 
  labs(
    x = "", 
    y = "Net farm income", 
    title = "Net farm income", 
    caption = "The red diamond represents the mean and the error bars show the distribution around the mean") + 
  theme(plot.caption = element_text(hjust = 0))
```

## Operating Profit Margin - Leafy Green operations

```{r opm_green_boxplot, fig.align="center", fig.width=7, fig.height=6, out.width="100%"}

# OPM
opm <- sum %>% 
  filter(yvar=="opm")
  
ggplot(opm, aes(x = type, y = mean)) +
  geom_point(size = 4, shape = 23, fill = "red") + 
  geom_errorbar(aes(ymin = mean - se,
                    ymax = mean + se, 
                    width = 0.2)) + 
